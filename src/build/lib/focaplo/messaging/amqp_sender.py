...
class Sender(object):
    ...
    def send(self, message, host, port="5672", user="guest", password="guest", virtualhost="/", exchange="temp", queue="test_queue", routingkey="test"):
        ...
        chan = None
        try:
            print 'send messsage to ' + host + ':' + port + ' virtual host:' + virtualhost + ' exchange:' + exchange + ' queue:' + queue + ' routing:' + routingkey
...
def main(argv=None):
    ...
    try:
        ...
        print 'usage: message Sender host port user password virtualhost exchange queue routingkey '
        sys.exit(2)
    if len(args) < 2:
        print 'usage: message Sender message host [port user password virtualhost exchange queue routingkey ]'
        sys.exit(2)
